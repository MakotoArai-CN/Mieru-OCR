  ...
  observer = new MutationObserver((mutations) => {
    ...
    for (const mutation of mutations) {
      if (mutation.type === 'childList') {
        ...
        if (mutation.target instanceof SVGElement) scheduleAutoSolve();
      } else if (mutation.type === 'attributes') {
        const target = mutation.target;
        if (target instanceof HTMLElement) {
          if (target instanceof HTMLImageElement && (mutation.attributeName === 'src' || mutation.attributeName === 'data-src' || mutation.attributeName === 'srcset')) scheduleAutoSolve();
          else if (target instanceof HTMLCanvasElement) scheduleAutoSolve();
          else if (target instanceof SVGElement) scheduleAutoSolve();
          else if (mutation.attributeName === 'style' && target.style.backgroundImage) scheduleAutoSolve();
        }
      }
    }
