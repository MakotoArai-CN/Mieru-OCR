    ...
    if (matchedRule) {
      ...
      if (element) {
        ...
        if (matchedRule.inputSelector) {
          inputEl = document.querySelector(matchedRule.inputSelector) as HTMLInputElement;
        }
        if (!inputEl) {
...
async function handleRecognize(captchaId: string, sendResponse: (response: any) => void): Promise<void> {
  ...
  try {
    ...
    if (customCaptchaElement) {
      const rect = customCaptchaElement.getBoundingClientRect();
      captcha = {
        id: 'custom-selected',
        type: customCaptchaElement.tagName.toLowerCase() === 'img'
          ? 'image'
          : customCaptchaElement.tagName.toLowerCase() === 'canvas'
            ? 'canvas'
            : 'svg',
        element: customCaptchaElement,
        rect,
        confidence: 100,
        inputElement: customInputElement,
        elementInfo: {
          tagName: customCaptchaElement.tagName.toLowerCase(),
          id: (customCaptchaElement as HTMLElement).id || null,
          className: (customCaptchaElement as HTMLElement).className?.toString?.() || '',
          width: Math.round(rect.width),
          height: Math.round(rect.height),
          src: (customCaptchaElement as HTMLImageElement).src,
        },
      };
    } else {
...
function handleGetStatus(sendResponse: (response: any) => void): void {
  ...
  const status = {
    ...
    isReady: !!(customCaptchaElement || captchas.length > 0) && !!(customInputElement || currentCaptcha?.inputElement),
  };
  ...
  initElementPicker('captcha', async (result) => {
    ...
    if (result.success) {
      ...
      if (!customInputElement) {
        startGuessMode('input', result.element);
      } else {
    ...
    for (const guessed of guessedElements) {
      if (guessed.element === target || guessed.element.contains(target)) {
        ...
        if (mode === 'captcha') {
          ...
          const rect = guessed.element.getBoundingClientRect();
          currentCaptcha = {
            id: 'guessed-selected',
            type: guessed.element.tagName.toLowerCase() === 'img'
              ? 'image'
              : guessed.element.tagName.toLowerCase() === 'canvas'
                ? 'canvas'
                : 'svg',
            ...
            inputElement: customInputElement,
            elementInfo: {
              tagName: guessed.element.tagName.toLowerCase(),
              id: (guessed.element as HTMLElement).id || null,
              className: (guessed.element as HTMLElement).className?.toString?.() || '',
              width: Math.round(rect.width),
              height: Math.round(rect.height),
              src: (guessed.element as HTMLImageElement).src,
            },
          };
...
async function saveAndRecognize(captchaSelector?: string, inputSelector?: string): Promise<void> {
  if (!customCaptchaElement && !currentCaptcha) {
    Logger.warn('无验证码元素，无法保存规则');
    return;
  }
  const selector = captchaSelector || detector.generateSelector(customCaptchaElement || currentCaptcha!.element);
  const inputSel = inputSelector || (customInputElement ? detector.generateSelector(customInputElement) : undefined);
  try {
    ...
    if (customCaptchaElement && customInputElement) {
      setTimeout(() => {
        tryAutoSolveOnce();
      }, 300);
    }
...
async function handlePreviewCaptcha(captchaId: string, sendResponse: (response: any) => void): Promise<void> {
  try {
    ...
    if (customCaptchaElement) {
      ...
      captcha = {
        id: 'preview',
        type: customCaptchaElement.tagName.toLowerCase() === 'img' ? 'image' : 'canvas',
        element: customCaptchaElement,
        rect,
        confidence: 100,
        inputElement: null,
