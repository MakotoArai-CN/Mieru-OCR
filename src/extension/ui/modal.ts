...
function ensureStyles(): void {
    if (document.getElementById('Mieru-ui-modal-styles')) return;

    const style = document.createElement('style');
    style.id = 'Mieru-ui-modal-styles';
    style.textContent = `
.Mieru-ui-modal-overlay {
position: fixed;
inset: 0;
...
.Mieru-ui-modal-title {
font-size: 14px;
font-weight: 600;
color: var(--text-primary, #1a1a2e);
line-height: 1.2;
}
.Mieru-ui-modal-close {
border: none;
...
.Mieru-ui-modal-btn:hover { background: var(--bg-hover, #d0e2f5); }
.Mieru-ui-modal-btn.primary {
background: var(--primary, #4A90E2);
border-color: var(--primary, #4A90E2);
color: #fff;
}
.Mieru-ui-modal-btn.primary:hover { background: var(--primary-hover, #357ABD); }
@media screen and (max-width: 768px) {
.Mieru-ui-modal { border-radius: 12px; }
.Mieru-ui-modal-actions { flex-direction: column-reverse; }
.Mieru-ui-modal-btn { width: 100%; min-height: 44px; font-size: 14px; }
}
`;
...
function createBaseModal(title: string): {
    ...
    overlay.className = 'Mieru-ui-modal-overlay';

    const modal = document.createElement('div');
    modal.className = 'Mieru-ui-modal';

    const header = document.createElement('div');
    header.className = 'Mieru-ui-modal-header';

    const titleEl = document.createElement('div');
    titleEl.className = 'Mieru-ui-modal-title';
    titleEl.textContent = title;

    const closeBtn = document.createElement('button');
    closeBtn.className = 'Mieru-ui-modal-close';
    ...
    body.className = 'Mieru-ui-modal-body';

    const actions = document.createElement('div');
    actions.className = 'Mieru-ui-modal-actions';
...
export function showAlert(options: AlertOptions): Promise<void> {
    return new Promise((resolve) => {
        ...
        okBtn.className = 'Mieru-ui-modal-btn primary';
        okBtn.type = 'button';
...
export function showConfirm(options: ConfirmOptions): Promise<boolean> {
    return new Promise((resolve) => {
        ...
        cancelBtn.className = 'Mieru-ui-modal-btn';
