...
| 浏览器扩展 | 仓库安装            | [Github](https://github.com/MakotoArai-CN/Mieru-OCR/releases/latest)                   | 直接下载压缩包文件，解压后安装浏览器扩展              |
...
2. 安装脚本: 在[Greasy Fork](https://greasyfork.org/)脚本市场、[ScriptCat](https://www.scriptcat.org/)脚本市场中搜索 "Mieru-OCR" 安装
...
1. 下载模型文件:
   - [common.onnx](https://github.com/MakotoArai-CN/Mieru-OCR/releases/latest/download/common.onnx)
   - [charsets.json](https://github.com/MakotoArai-CN/Mieru-OCR/releases/latest/download//charsets.json)
2. 放到你能找到的目录，后续导 入到脚本中（需要开启扩展的 **允许访问文件URL** ）
3. `git clone https://github.com/MakotoArai-CN/Mieru-OCR.git && cd Mieru-OCR`，构建: `bun run install && bun run build`
4. 安装 `dist\userscript\Mieru-OCR.user.js`，浏览器扩展开启开发者模式后，选择目录`dist\extension`即可安装
5. 等待脚本加载完毕，打开设置页设置白名单（必须）

> 1.1.3版本开始添加量化模型，模型体积减少约48%，可以在1.1.3版本的Release中下载
...
1. 访问任意网页
2. 右上角出现 "Mieru-OCR" 面板
3. 点击扩展，找到 "Mieru-OCR" 面板的**打开设置**，配置脚本白名单
...
- 1.2.2
  ...
  - 实验性支持滑块验证码以及文字点选，在本次开发测试中发现每一个站点的滑块，文字点选等验证码都会不一样，期间遇到的简单的纯图像，也有图像经过编码，裁切打乱等，通用性不强，只能作为实验性功能，后续任需要大量测试。
  - 修复站点有列表时会出现自动拉到最底部的bug
  - 移除量化模型文件，目前模型量化工作还需进一步调试（其实是懒），后续会作为新项目发出
...
- 1.1.6
  ...
  - benchmark 新增稳定数据集导出能力（`--dataset-dir` / `captchas.json`）
  - 默认配置补齐高级关键词与交互式能力相关字段
  - 设置页高级规则改为 chips 交互
  - 内置关键词支持删除，并可一键重置
...
- [Linux.do](https://linux.do/) - 强大的开源交流社区。
...
[![Star History Chart](https://api.star-history.com/svg?repos=MakotoArai-CN/Mieru-OCR&type=date&legend=top-left)](https://www.star-history.com/#MakotoArai-CN/Mieru-OCR&type=date&legend=top-left)
